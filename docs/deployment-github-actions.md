...
数据链接不是固定到某一期。部署时会动态发现最新官方文件：

- e-Stat GDP / CPI / 实际工资：从 e-Stat 搜索页解析最新 `file-download` 链接。
- e-Stat 在留外国人和外国人工资：从对应 e-Stat 数据集页解析候选文件，并校验文件结构后下载。
- 厚生労働省外国人雇用状况：从索引页选择最新年度结果页，再解析 XLSX 附件。

如果动态发现失败，程序才会使用备用链接，并把结果写入 `/sources`。
